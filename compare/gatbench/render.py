...
def parse(path):
    ...
    # Median, not mean: one count landing on a busy moment should not
    # drag the reported figure. Run-to-run spread on a shared box runs
    # to several percent, which is why the spread is reported too.
    return {
        name: (
            statistics.median(v[0] for v in vals),
            round(statistics.median(v[1] for v in vals)),
            round(statistics.median(v[2] for v in vals)),
...
def spread(data):
    """Worst observed min-to-max swing across counts, as a percentage.
    A reader needs this to know how many digits of a ratio are real."""
    worst = 0.0
    for _, _, _, (lo, hi) in data.values():
        if lo > 0:
            worst = max(worst, (hi - lo) / lo * 100)
    return worst
...
def build_block(data):
    parts = [
        BEGIN,
        f"_Generated by `compare/gatbench/report.sh` on {cpu_model()},"
        f" {go_version()}, load {load_avg()}. Median of 5 runs; worst"
        f" run-to-run spread {spread(data):.0f}%._",
        "",
        ...
        " connect-go service on this workload\" — and only to about one"
        " decimal place: run-to-run noise moves each side a few percent"
        " independently, so a ratio can shift by ~0.2× between runs"
